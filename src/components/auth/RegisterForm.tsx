...
import { Loader2, User, Mail, Phone, Lock, AlertCircle } from "lucide-react"
    ...
    const onSubmit = async (data: z.infer<typeof registerSchema>) => {
        ...
        try {
            ...
            if ((axios as any).isAxiosError(e) && e.response) {
                console.log("Registration Error Response:", e.response.data) // Added for debugging
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                const data = e.response.data as any

                // Handle field-specific errors (Django Rest Framework format)
                if (typeof data !== 'string' && !data.error) {
                    // It's likely a validation error dict like { email: ["Error"] }
    ...
    return (
        <div className="w-full max-w-md p-8 space-y-6 bg-white rounded-xl shadow-lg border border-gray-100">
            <div className="text-center">
                ...
                <p className="text-sm text-gray-500 mt-2">Join Shambit for spiritual journeys</p>
            </div>
